...
const (
	AgentService_ReportMetrics_FullMethodName  = "/monitoring.agent.AgentService/ReportMetrics"
	AgentService_GetAgentStatus_FullMethodName = "/monitoring.agent.AgentService/GetAgentStatus"
	AgentService_UpdateConfig_FullMethodName   = "/monitoring.agent.AgentService/UpdateConfig"
)
...
func (c *agentServiceClient) ReportMetrics(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[MetricsData, ReportMetricsResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AgentService_ServiceDesc.Streams[0], AgentService_ReportMetrics_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[MetricsData, ReportMetricsResponse]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentService_ReportMetricsClient = grpc.ClientStreamingClient[MetricsData, ReportMetricsResponse]
...
func (UnimplementedAgentServiceServer) ReportMetrics(grpc.ClientStreamingServer[MetricsData, ReportMetricsResponse]) error {
...
func _AgentService_ReportMetrics_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(AgentServiceServer).ReportMetrics(&grpc.GenericServerStream[MetricsData, ReportMetricsResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentService_ReportMetricsServer = grpc.ClientStreamingServer[MetricsData, ReportMetricsResponse]
...
var AgentService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "monitoring.agent.AgentService",
